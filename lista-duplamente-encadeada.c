...
void criar_primeiro_no(int valor){
    ...
    // modo de se alocar espaço para um novo nó, seguindo a fórmula: tipo_objeto *nome_objeto = (tipo_objeto*) malloc(sizeof(tipo_objeto))
    // o asterisco (*) aparece na primeira vez para declarar que o objeto é um ponteiro e na segunda vez para fazer o casting de um tipo de ponteiro
...
void inserir_meio(int valor, int posicao){
    ...
    // ponteiro descartável que começa com o valor da cabeça. é usado para navegar pela lista sem mudar o valor da cabeça (é usado em várias funções)
    estrutura_no *ponteiro_atual = cabeca;

    // posicao é a variável que determina em qual posicao da lista o novo nó irá ficar; considera-se a primeira posição como 1 ao invés de 0 (classroom que pediu)
    // se a posição for 1 (inicio do nó) ou menor (nem sequer existe na lista) a função retorna sem inserir nada
    if(posicao <= 1){
        return;
    }

    // a variável posicao vai ser decrementada aqui, o loop é executado enquanto ela for maior que 1 (enquanto ainda não se chegou na posição de inserção)
    while(posicao > 1){
        // se o ponteiro atual não for a cauda da lista, o loop continua, caso contrário, a função retorna sem inserir nada (ela não insere no final da lista)
    ...
    // é a inserção mais difícil de entender por ser a com mais mudanças nos ponteiros anteriores e posteriores, mas faz um esforço que tu entende (ou faz o L)
...
void remover_no(int posicao){
    ...
    // agora que o nó foi removido, free() é chamado para liberar esse espaço de memória que antes estava sendo ocupado (essa função é chamada mais vezes)
    free(ponteiro_atual);
}
...
// busca e exibe qual o primeiro nó que possui o valor desejado pelo usuário (ou se nenhum nó possui esse valor)
void buscar(int objetivo){

    // contador que começa em 1 para caso o objetivo (valor que queremos encontrar) esteja logo no primeiro nó
    int contador = 1;
    estrutura_no *ponteiro_atual = cabeca;
...
// listamos os nós: número do nó, endereço do nó, endereço anterior ao nó, endereço posterior ao nó e valor do nó
void listar(){
...
// função wrapper usada para interagir com o usuário e chamar outras funções que efetivamente fazem coisas (modo simples do usuário interagir com o programa)
void insercao_no(){
    ...
    do {
        printf("Insira em qual posição da lista você deseja inserir um nó (Insira uma posição que exista na lista ou a posição imediatamente posterior à última. A lista começa por 1): ");scanf("%d", &posicao);
...
void remocao_no(){
    ...
    do {
        printf("Insira a posição do nó que você deseja remover (Insira uma posição que exista na lista. A lista começa por 1): ");scanf("%d", &posicao);
    } while(posicao < 1 || posicao > contador);

    if(posicao == 1 && contador > 1){
        remover_no_cabeca();
    } else if(contador == 1){
        printf("Você está sem nós! Crie um para continuar. Insira seu valor (CTRL+C PARA SAIR): ");scanf("%d", &valor_descartavel);
        free(cabeca);
...
int main(){
    ...
    printf("Bem-vindo! Crie seu primeiro nó dando um valor inteiro a ele (CTRL+C PARA SAIR): ");scanf("%d", &valor_descartavel);
    criar_primeiro_no(valor_descartavel);
    printf("Agora com seu primeiro nó, diga o que quer fazer!\n");

    // esse loop se repete até que o usuário decida deixar o programa
    while(1){
        printf("[1] Inserir nó\n[2] Remover nó\n[3] Buscar primeira aparição de um valor em um nó\n[4] Listar nós\n[5] Sair\nResposta: ");scanf("%d", &resposta);
        
        switch(resposta){
            case 1: insercao_no(); break;
            case 2: remocao_no(); break;
            case 3: printf("Insira o valor que você deseja buscar: ");scanf("%d", &valor_descartavel); buscar(valor_descartavel); break;
            case 4: listar(); break;
            case 5: exit(0); break;
            default: break;
        }
